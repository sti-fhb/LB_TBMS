...
- **離線由 API 呼叫結果判定**：不做獨立 ping；**LBSB01 呼叫 APILB 失敗才知道當下是離線**。離線時啟動 Timer **每 3 分鐘**、或使用者按主畫面 **[更新]** 時執行同步；當 Call API 成功 → 回復線上（R03）
- **Local-first 寫入**：不管線上或離線，Call APILB 之前一律先寫 Local DB；上線後 Sync 更新採「**一律以 Local DB 蓋中央 DB**」（離線時可修改印表機、列印測試標籤）（R03）
...
**畫面狀態指示**：標題列 + 狀態標籤顯示線上（綠）/離線（紅）。離線時由 Timer **每 3 分鐘**、或使用者按主畫面 **[更新]** 觸發同步；同步成功（Call APILB 回 200）即靜默切換線上（R03）。
