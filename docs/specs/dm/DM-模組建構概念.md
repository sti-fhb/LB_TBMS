...
DM（Document Management）模組提供 **SOP 文件、操作手冊、訓練教材**（文件型）三類檔案的**集中版本控管、電子簽核、發布通知**，並對外（主系統、Moodle、郵件）提供穩定的查閱介面。

> **教材分流**：訓練教材中的**文件型**素材（PDF/PPT/圖文）由 DM 管版本，ET 模組課程以連結引用；**影片型**教材仍由 ET 模組自管（保留 RQET004 強制觀看追蹤能力）。表單不在 DM 範圍。
...
- **權責分離**：文件簽核角色（**SOP_REVIEWER** 唯一硬卡點；其餘任一 SS 角色視為一般使用者）與業務角色（護理師、技術師）獨立管理
...
| 認證 | 透過 **SS API**（APISS001 / APISS002）換取 Token（無獨立帳號表，亦不從 DP 同步）| 獨立 SS 模組為 IdP |

### 3.2 部署拓撲

- **獨立服務**：DM 後端與前端獨立部署（可同 repo 不同 service，或獨立 repo）
- **網路**：與主系統後端同網段（或 service mesh），可直連 SS API（認證）與 DP API（系統參數 / 共用稽核）
...
| # | 接點 | 方向 | 技術 |
|---|------|------|------|
| **1** | DM 認證 | DM → SS | 呼叫 **APISS001** 以 SS 帳密換 Token + 角色 + 可用功能 |
| **2** | Token 驗證 | DM → SS | 每次請求呼叫 **APISS002** 驗 Token |
| **3** | 系統參數查詢 | DM → DP | 呼叫 **APIDP003** 取系統參數（`DM_UPLOAD_PATH` 等，內建靜態 Token）|
| **4** | 稽核 Log（共用）| DM → DP | 跨系統重大事件呼叫 SRVDP003 寫入 DP_AUDIT；DM 自身審查軌跡寫 DM_AUDIT |
| **5** | 主系統 → 線上操作手冊 | 主系統 → DM | 各畫面以 func_name call DM API 開啟手冊 |
| **6** | Moodle → SOP / 手冊 | Moodle → DM | 教材內 HTML 超連結指向 `https://dm.tbms.internal/doc/{DOC_ID}` |
| **7** | 通知發送 | DM → 外部 | SMTP（DM 自有）或主系統通知中心 |

### 4.3 明確不做的事

- ❌ DM 不維護自己的使用者帳號表（完全依賴 **SS 模組**，與主系統 DP 帳密無關）
- ❌ DM 不直接讀主系統業務 DB（如捐血人、血品）
- ❌ 主系統不直接讀 DM 的 DB（所有讀取走 DM API）
- ❌ Moodle 不直接讀 DM 的 DB（只透過 URL 超連結 + SS session）
...
階段 1：DM 最小可用版（MVP）
  ...
  ├─ **SS 認證整合**（APISS001 / APISS002）+ DP 系統參數整合（APIDP003）
  ├─ 文件上傳 / 下載 / 版本列表
  └─ 最小文件瀏覽 UI
...
### 6.1 DM → SS（認證 / Token 驗證）

| 編號 | 方向 | 用途 | 備註 |
|---|---|---|---|
| **APISS001** | DM → SS | 以 SS 帳密換取 Token + user info（含角色與可用功能）| 與 Moodle 共用同一支認證 API |
| **APISS002** | DM → SS | 每次請求驗 Token 是否有效 | 與 Moodle 共用 |

### 6.1b DM → DP（系統參數 / 共用稽核）

| 編號 | 方向 | 用途 | 備註 |
|---|---|---|---|
| APIDP003 | DM → DP | 取系統參數（`DM_UPLOAD_PATH` 等）| DM 內建靜態 Token，啟動時呼叫 |
| SRVDP003 | DM → DP | 跨系統重大事件寫共用 Audit Log（既有服務）| 現成；DM 自身審查軌跡寫 DM_AUDIT |
...
- **APISS001 + APISS002** （SS 認證 + Token 驗證）
...
- **認證模組**：SS 模組規格位於主專案 `TBMS/docs/specs/ss/spec.md`（DM 與 ET 共用）
