...
## 8. 本日工作總結（2026-04-22，SA 側紀錄）

### 8.1 文件對齊（主軸）

以 EA UCLB 下 Diagram + 離線原則 Rule 為準，全面修正 SA 文件：

| 檔案 | 變更摘要 |
|------|---------|
| `spec.md` | L17-L22 核心特性改 — 移除健康檢查、Timer 60s→3min、以 Local DB 蓋中央 DB |
| `LBSB01-開發者指南.md` | §2.4 / §4 / §5 / §6 / §7 / §8 / §9 全面改寫；§6.4 重寫為 OffLine Retry Timer + [更新] 雙觸發；§6.9 衝突處理改為「一律以 Local 蓋中央」 |
| `LBSB01-操作手冊.md` | §2.1~2.4 / 附錄 A：移除健康檢查敘述、補 [更新] 按鈕、Timer 3 分鐘 |
| `contracts/srv-contracts.md` | 移除「健康檢查端點」小節，改為「離線偵測規則」|
| `SRVLB001-標籤列印整合指南.md` | 時序圖改引 UCLB001 Drill-Down CompositeStructure + contracts |

### 8.2 EA 模型調整

| Diagram / Element | 動作 |
|------|------|
| `LBL001-標籤印出案例` Diagram | 改名為 `UCLB001-標籤列印使用案例`（與 Parent UseCase 同名）|
| 同 Diagram Type | `Use Case` → `Activity`（SA 手動 GUI，MCP 唯讀無法改）|
| `ET-教育訓練 UseCase` Diagram | 改名為 `ET-教育訓練`（單一 diagram 與 Parent 同名）|
| `接收列印指令 StepN` → `LBSB01 專用Token` Data | 加 `Dependency<<IN>>`（connector_id `1776848339`）|
| `Activity<<Data>>` `Online Queue` / `Offline Queue` | 新建（element_id `588021784` / `-1650599665`），UCLB101 兩個 QUEUE Step 的 Classifier 指向之 |

### 8.3 Images（PNG）

| 處理 | 檔案 |
|------|------|
| 由 EA 重匯 | `uclb001-flow` / `uclb002-flow` / `uclb002-usecase` / `uclb101-flow` / `srvlb001-architecture` |
| 刪除（原 mermaid 衍生，無 EA 對應）| `local-sync-flow` / `offline-reconnect` / `queue-state-machine` / `srvlb001-flow` / `srvlb001-sequence` / `srvlb001-sequence-a` / `task-listener-flow` |
| 刪除 mermaid 源檔 | 全部 7 個 `.mmd` |
| 刪除 leftover SVG | 4 個 `docs/usecase/UCLB*.svg` |
| 主專案同步 | 複製 4 張 UCLB PNG 到 `C:\TSBMS\docs\specs\lb\images\` |

### 8.4 ClassifierTagSync EA Add-in 修復（關聯性：render cache 殘缺原因）

SA 匯出的 PNG 長期「少 code」的真正原因 — Add-in 三個 Bug：

| 層級 | 原因 | 修法 |
|------|------|------|
| 組建層 | `.csproj` 未 EmbedInteropTypes → DLL 硬綁 `Interop.EA v2.10.238.1` | `<EmbedInteropTypes>true</EmbedInteropTypes>` + `Microsoft.CSharp` 參考 |
| 執行層 | 靜態 `Dictionary<,>` 在 ShapeScript 多執行緒 callback 下有 race → AV | 改為 `ConcurrentDictionary<,>` |
| 收尾層 | `EA_Disconnect` 內 `GC.Collect() + WaitForPendingFinalizers()` 觸發 RCW double-free | 全部移除，只留 `ClearCache()` |

另補強：`TaggedValues.GetAt(i)` 迴圈加 per-iteration try/catch，單筆 Tag 讀取失敗不中斷整批。

**驗證**：原 32-bit PowerShell 呼叫 `EA_Connect` 會因 Interop.EA 版本找不到而炸；修好後 `EA_Connect(null)` / `EA_Disconnect()` 全通過。

### 8.5 版控調整

| 變更 | Repo | Commit / Remark |
|------|------|----------------|
| 新增 repo | `sti-fhb/EA_TOOL`（local `C:\TSBMS\EA_TOOL\`）| `a21cad3` 初始化 — Addin 源碼 + SPEC Pattern + VBS 腳本 |
| 搬目錄 | `_LB/issue/` → `_LB/docs/issue/` | `ISLB001~004` 統一放 `docs/issue/`（更正先前 `b2b36c0` commit 的誤放） |
| ISLB004 建立 | `sti-fhb/LB_TBMS` | 含實作範圍 + §5 EA 完成矩陣 + §6 業務邏輯 10 條 + §7 驗收 checklist |

### 8.6 重要經驗（給未來的 SA / PG）

1. **EA render cache 只能靠「完全結束 EA.exe」清除**。MCP 的 `close_project` / `reload_diagram` / `clear_dd_cache` 都清不到。改 Add-in / Stereotype / Tagged Value / Classifier 後匯出 PNG，若 `git status` 沒 bytes 變化 — 代表 cache 殘留，要請 SA 徹底重啟 EA。
2. **ClassifierTagSync 未納入 git 前的版本**會讓 ShapeScript 顯示 Classifier Tag 失敗，連帶讓 SA 匯出的 PNG 看起來「少 code / 缺欄位名」。若 PG 閱讀 PNG 覺得「欄位名稱怎麼空白」，優先懷疑是否 Add-in 未載入。
3. **`dotnet build` 不能用於 ClassifierTagSync**（`RegisterForComInterop` 需要 `RegisterAssembly` MSBuild task，.NET Core MSBuild 不支援）。必須用 `Program Files\Microsoft Visual Studio\...\MSBuild.exe`。
4. **Issue 與主專案的編碼不互通**：LB issue 用 `ISLB{3 位}`，主專案的 issue 另走其系統。不要把兩邊號碼混用。
5. **LB_PRINTER 保留字 `"USB"`** 代表本機 USB 直連，PG 實作 SRVDP010 / APILB007 時必須特別處理此值（跳過 LB_PRINTER 驗證）。
